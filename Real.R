...
# API key for Ambee
API_KEY <- "3de0b3998988a16cbb1fbb9fdbcbeee0cc8e47d325c74c985ff14860247fbcfe"
...
server <- function(input, output, session) {
  ...
  output$sunrise_sunset <- renderText({
    ...
    # Sunrise/Sunset timestamps (in UTC)
    sunrise <- as.POSIXct(city$sunrise, origin = "1970-01-01", tz = "UTC")
    sunset  <- as.POSIXct(city$sunset,  origin = "1970-01-01", tz = "UTC")
    
    # Format time in local timezone (you can change to "Asia/Kolkata" if needed)
    paste("🌅 Sunrise:", format(sunrise, "%H:%M %p"), "| 🌇 Sunset:", format(sunset, "%H:%M %p"))
  })
  ...
  output$weather_summary <- renderTable({
    ...
    forecasts %>%
      group_by(date) %>%
      summarize(
        `Avg Temp (°C)` = round(mean(temp), 1),
        `Min Temp (°C)` = round(min(temp_min), 1),
        `Max Temp (°C)` = round(max(temp_max), 1),
        `Avg Humidity (%)` = round(mean(humidity), 1),
        `Avg Wind Speed (m/s)` = round(mean(wind_speed), 1),
        `Most Common Weather` = names(which.max(table(weather_main)))
      )
  })
